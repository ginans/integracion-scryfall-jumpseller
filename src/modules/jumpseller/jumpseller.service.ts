...
import {
  ...
  ICreateImageRequest,
} from './interfaces/create-image.interface';
...
import {
  JumpsellerUpdateVariantResponse,
  JumpsellerUpdateVariantResponseError,
} from './interfaces/variants-jumpseller/jumpsellerUpdateVariantResponse.interface';
...
import { createCustomFieldRequest } from './interfaces/custom-fields-jumpseller/createCustomfieldRequest.interface';
import { CreateCustomFieldResponse } from './interfaces/custom-fields-jumpseller/createCustomFieldResponse.interface';
import { UpdateCustomFieldRequest } from './interfaces/custom-fields-jumpseller/updateCustomFieldRequest.interface';
import { UpdateCustomFieldResponse } from './interfaces/custom-fields-jumpseller/updateCustomFieldResponse.interface';
...
export class JumpsellerService {
  ...
  private registerError(error: any): void {
    if (error.response) {
      ...
      this.logger.error(
        `Encabezados de respuesta: ${JSON.stringify(error.response.headers)}`,
      );
    }
  }
  async createProduct(
    product: JumpsellerProductRequest,
  ): Promise<JumpsellerProductResponse> {
    try {
      this.logger.debug(
        `Enviando solicitud a Jumpseller: ${product.product.sku}`,
      );
      const { data } = await this.client.post<JumpsellerProductResponse>(
        JumpsellerEndpoints.PRODUCTS,
        product,
      );
      return data;
    } catch (error) {
      this.logger.error(
        `❌ Error al crear producto en Jumpseller: ${error.message}`,
      );
      this.registerError(error);
      throw error; // Lanzar el error para que no retorne undefined
    }
  }
  async getJumpsellerProductById(
    productId: number,
  ): Promise<JumpsellerProductResponse> {
    try {
      const { data } = await this.client.get<JumpsellerProductResponse>(
        `${JumpsellerEndpoints.PRODUCT}/${productId}.json`,
      );
      return data;
    } catch (error) {
      this.logger.error(
        `❌ Error al traer el producto ${productId} de Jumpseller: ${error.message}`,
      );
      this.registerError(error);
      throw error;
    }
  ...
  async insertImages(
    ...
    try {
      const { data } = await this.client.post<ICreateImageResponse>(
        `${JumpsellerEndpoints.PRODUCT}/${productId}/${JumpsellerEndpoints.IMAGES}`,
        images,
      );
      return data;
    } catch (error) {
      this.logger.error(
        `❌ Error al insertar imágenes en Jumpseller: ${error.message}`,
      );
      this.registerError(error);
    }
  }
  async createCustomField(
    ...
    try {
      const { data } = await this.client.post<CreateCustomFieldResponse>(
        JumpsellerEndpoints.CUSTOM_FIELDS,
        customField,
      );
      return data;
    } catch (error) {
      this.logger.error(
        `❌ Error al crear campo personalizado en Jumpseller: ${error.message}`,
      );
      this.registerError(error);
    }
  }
  async updateCustomField(
    ...
    try {
      const { data } = await this.client.put<UpdateCustomFieldResponse>(
        `${JumpsellerEndpoints.UPDATE_CUSTOM_FIELDS}/${productId}.json`,
        customField,
      );
      return data;
    } catch (error) {
      this.logger.error(
        `❌ Error al actualizar campo personalizado en Jumpseller: ${error.message}`,
      );
      this.registerError(error);
    }
  }
  ...
  async getAllCustomFields(): Promise<GetAllCustomFieldResponse> {
    try {
      const { data } = await this.client.get<GetAllCustomFieldResponse>(
        JumpsellerEndpoints.CUSTOM_FIELDS,
      );
      return data;
    } catch (error) {
      this.logger.error(
        `❌ Error al obtener campos personalizados en Jumpseller: ${error.message}`,
      );
      this.registerError(error);
    }
  }
  ...
  async addStock(
    stock: StockJumpsellerRequest,
  ): Promise<StockJumpsellerResponse> {
    try {
      const { data } = await this.client.put<StockJumpsellerResponse>(
        JumpsellerEndpoints.STOCK,
        stock,
      );
      return data;
    } catch (error) {
      this.logger.error(
        `❌ Error actualizar stock en Jumpseller: ${error.message}`,
      );
      this.registerError(error);
      if (error.response)
        return {
  ...
  async createJumpsellerVariant(
    ...
    try {
      const { data } = await this.client.post<JumpsellerCreateVariantResponse>(
        `${JumpsellerEndpoints.PRODUCT}/${productId}/${JumpsellerEndpoints.VARIANTS}`,
        variantReq,
      );
      return data;
    } catch (error) {
      this.logger.error(
        `❌ Error al crear variante en Jumpseller: ${error.message}`,
      );
      this.registerError(error);
    }
  }
  ...
  async updateVariant(
    productId: number,
    variantId: number,
    variant: JumpsellerUpdateVariantRequest,
  ): Promise<
    JumpsellerUpdateVariantResponse | JumpsellerUpdateVariantResponseError
  > {
    try {
      const { data } = await this.client.put<JumpsellerUpdateVariantResponse>(
        `${JumpsellerEndpoints.PRODUCT}/${productId}/${JumpsellerEndpoints.VARIANT}/${variantId}`,
        variant,
      );
      return data;
    } catch (error) {
      this.logger.error(
        `❌ Error al actualizar variante en Jumpseller: ${error.message}`,
      );
      this.registerError(error);
      if (error.response)
