...
export class VariantsService {
  constructor(
    ...
    @InjectModel(StockAndSalesHistory.name)
    private readonly stockDiscountAndSalesHistoryModel: Model<StockAndSalesHistoryDocument>,
  ...
  async findAllVariants(query: PaginationQueryDto) {
    ...
    if (priceUpdateStatus) {
      filters.priceUpdateStatus = {
        $regex: `^${priceUpdateStatus}$`,
        $options: 'i',
      };
    }

    if (stockUpdateStatus) {
      filters.stockUpdateStatus = {
        $regex: `^${stockUpdateStatus}$`,
        $options: 'i',
      };
    }

    if (jumpsellerStatus) {
      filters.jumpsellerStatus = {
        $regex: `^${jumpsellerStatus}$`,
        $options: 'i',
      };
    }
    ...
    try {
      const [variants, total] = await Promise.all([
        this.variantsModel
          ...
          .limit(limit)
          .exec(),
        this.variantsModel.countDocuments(filters).exec(),
      ]);
      ...
      throw new InternalServerErrorException(
        `Error trayendo variantes: ${error.message}`,
      );
    }
  }
  ...
  async saveStockFromFront(variant: CreateStockDto) {
    try {
      const existingVariant = await this.variantsModel.findOne({
      ...
      if (!existingVariant) {
        throw new Error(
          `Variant with variantId: ${variant.variantId} and productId: ${variant.productId} not found.`,
        );
      }
      const nullErrorMsg = null;
      await this.variantsModel.updateOne(
        {
          variantId: variant.variantId,
          productId: variant.productId,
        },
        {
          $set: {
            variantStock:
              parseInt(existingVariant.variantStock?.toString()) +
              parseInt(variant.variantStock?.toString()),
      ...
      this.logger.error(
        `Error al guardar stock desde el front: ${error.message}`,
      );
  ...
  async sendStockToJumpseller(variant: CreateStockDto) {
    try {
      const productVariant = await this.variantsModel.findOne({
        productId: variant.productId,
        variantId: variant.variantId,
      });
      ...
      this.logger.log(
        `🦍 Enviando stock a Jumpseller: ${JSON.stringify(stockRequest)}`,
      );
      ...
      if (response.status !== 201 && response.message) {
        ...
        this.logger.log(
          `Se actualizó el stock de la variante ${variant.variantId} en Jumpseller`,
        );
        ...
        throw new Error(
          `Error al enviar stock a Jumpseller: ${response.message}`,
        );
      }
    } catch (error) {
  ...
  async obtainVariantforPrices(
    ...
    try {
      let variantes: IVariant[] = [];
      // Buscar variantes según los parámetros recibidos
      if (variantId && productId) {
        const variante = await this.variantsModel
          .findOne({
        ...
        variantes = await this.variantsModel
          .find({
            isPriceUpdateable: true,
            game: game,
            rarity: rarity,
      ...
      throw new InternalServerErrorException(
        `Error al obtener la variante ${error.message}`,
      );
    }
  }

  async calculatePricesByVariant(
    variant: IVariant,
    type?: string,
  ): Promise<any> {
    try {
      // Obtener el precio del dólar
      const usdPriceDoc = await this.usdPriceModel.findOne({
        gameID: EnumGamePrefix.MAGIC,
      });
      if (!usdPriceDoc || !usdPriceDoc.usdPrice) {
        throw new Error(
          'No se encontraron valores del dolar para la colección Magic',
        );
      }
      const usdPrice = usdPriceDoc.usdPrice;
      // Obtener precios base por rareza
      const basePrice = await this.basePriceModel.findOne({
        game: EnumGame.MAGIC,
      });
      if (!basePrice) {
      ...
      let rarityPrices = {};
      if (type) {
        const rarityType = basePrice.basePrices.find(
      ...
      const matchingCard = await this.magicCardModel.findOne({
        idJumpSeller: variant.productId,
      });
      ...
      if (!matchingCard) {
        ...
        if (matchingCard.prices) {
          if (isNonFoil && matchingCard.prices.usd) {
            ...
            this.logger.warn(
              `No se encontraron precios para la carta ${matchingCard.oracleId} con finish ${variant.finish}`,
            );
          }
        } else {
          this.logger.warn(
            `No hay información de precios para la carta ${matchingCard.oracleId}`,
          );
        }
        ...
        let precioCLP =
          precioUSD === 0 || precioUSD === null ? 0 : precioUSD * usdPrice;
        precioCLP = Math.ceil(precioCLP / 100) * 100;
        const precioFinal =
          precioCLP === 0 ? 0 : Math.max(precioCLP, precioBaseRareza);
        ...
        if (precioFinal > 0) {
          ...
          const response = await this.jumpsellerService.updateVariant(
            variant.productId,
            variant.variantId,
            variantTo,
          );
      ...
      this.logger.error(
        `Error al calcular precios para la variante: ${error.message}`,
      );
      throw new InternalServerErrorException(
        `Error al calcular precios para la variante: ${error.message}`,
      );
  ...
  async savePricesFromFront(variant: CreatePricesDto) {
    try {
      ...
      const nullErrorMsg = null;
      const updatedVariant = await this.variantsModel.updateOne(
        { variantId: variant.variantId, productId: variant.productId },
        {
          $set: {
      ...
      if (updatedVariant.modifiedCount === 0) {
        throw new Error(
          `Failed to update variant with variantId: ${variant.variantId} and productId: ${variant.productId}.`,
        );
      }
      ...
      this.logger.error(
        `Error al guardar precios desde el front: ${error.message}`,
      );
      await this.updateVariantPriceStatus(
        variant.variantId,
        variant.productId,
  ...
  async sendPriceToJumpseller(variant: CreatePricesDto) {
    try {
      ...
      this.logger.error(
        `Error al enviar precio a Jumpseller: ${error.message}`,
      );
      await this.updateVariantPriceStatus(
        variant.variantId,
        variant.productId,
  ...
  async findVariantById(_id: string): Promise<IVariant> {
    if (!Types.ObjectId.isValid(_id))
      throw new BadRequestException('Formato de ID inválido');
    const variant = await this.variantsModel
      .findOne({ _id: new Types.ObjectId(_id) })
      .exec();
    if (!variant) throw new NotFoundException('Variante no encontrada');
    const variantResponse = variant as unknown as IVariant;
  ...
  async findByVariantId(
    ...
    try {
      const variant = await this.variantsModel
        .findOne({ variantId })
        .exec();
      return variant as unknown as IVariant;
    } catch (error) {
      this.logger.error(`Error al buscar variante por ID: ${error.message}`);
      throw new InternalServerErrorException(
        `Error al buscar variante por ID: ${error.message}`,
      );
    }
  }
  ...
  async updateVariantById(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        `Error al actualizar variante: ${error.message}`,
      );
    }
  }
  ...
  async updateAllIsPriceUpdateable(isPriceUpdateable: boolean) {
    try {
      await this.variantsModel.updateMany(
        {},
        {
          $set: {
      ...
      return {
        message: `Se actualizaron todos los isPriceUpdateable a ${isPriceUpdateable}`,
      };
    } catch (error) {
      this.logger.error(
        `Error al actualizar isPriceUpdateable: ${error.message}`,
      );
      throw new InternalServerErrorException(
        `Error al actualizar isPriceUpdateable: ${error.message}`,
      );
  ...
  async updateStock(order: IOrder) {
    ...
    for (const webhookProduct of order.products) {
      const variantToUpdate = await this.variantsModel
        .findOne({
      ...
      if (!variantToUpdate) {
        this.logger.warn(
          `No se encontró la variante para el producto con ID: ${webhookProduct.id} y variantId: ${webhookProduct.variant_id}`,
        );
      ...
      if (existingOrder) {
        this.logger.warn(
          `La orden con ID: ${order.id} ya existe en la base de datos`,
        );
        continue;
      }
      ...
      if (variantToUpdate && !existingOrder) {
        ...
        const newStock =
          variantToUpdate.variantStock > 0
            ? variantToUpdate.variantStock - webhookProduct.qty
            : 0;

        // Calcular el nuevo historySales (historial de ventas + cantidad vendida)
        const newHistorySales =
          (variantToUpdate.salesByCard || 0) + webhookProduct.qty;

        const stockDiscountAndSalesHistoryEntry =
          await this.stockDiscountAndSalesHistoryModel.create({
            orderId: order.id,
            productId: webhookProduct.id,
            variantId: webhookProduct.variant_id,
        ...
        this.logger.log(
          `stock actualizado para el id: ${webhookProduct.id}: el nuevo stock es ${newStock}, historySales: ${newHistorySales}`,
        );

        results.push(stockDiscountAndSalesHistoryEntry);
      } else {
